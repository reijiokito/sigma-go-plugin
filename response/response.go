...
Responses can be produced by sigma (e.g. an authentication plugin rejecting
a request), or proxied back from an Service’s response body.

Unlike sigma.ServiceResponse, this module allows mutating the response
...
// sigma.Response.GetStatus() returns the HTTP status code
// currently set for the downstream response (as an integer).
//
// If the request was proxied (as per sigma.Response.GetSource()),
// the return value will be that of the response from the Service
// (identical to sigma.ServiceResponse.GetStatus()).
//
// If the request was not proxied, and the response was produced
// by sigma itself (i.e. via sigma.Response.Exit()), the return value
// will be returned as-is.
func (r Response) GetStatus() (int, error) {
	return r.AskInt(`sigma.response.get_status`)
}

// sigma.Response.GetHeader() returns the value of the specified
...
// added by sigma (e.g. via sigma.Response.AddHeader()).
//
...
func (r Response) GetHeader(name string) (string, error) {
	return r.AskString(`sigma.response.get_header`, name)
}

// sigma.Response.GetHeaders() returns a map holding the response headers.
...
//
// Unlike sigma.ServiceResponse.GetHeaders(), this function returns
// all headers as the client would see them upon reception,
// including headers added by sigma itself.
//
...
func (r Response) GetHeaders(max_headers int) (res map[string][]string, err error) {
	if max_headers == -1 {
		return r.AskMap(`sigma.response.get_headers`)
	}

	return r.AskMap(`sigma.response.get_headers`, max_headers)
}

// sigma.Response.GetSource() helps determining where the current response
// originated from. sigma being a reverse proxy, it can short-circuit
...
// there has been a call to sigma.response.exit(). In other words, when the request
// was short-circuited by a plugin or by sigma itself (e.g. invalid credentials).
//
...
func (r Response) GetSource() (string, error) {
	return r.AskString(`sigma.response.get_source`)
}

// sigma.Response.SetStatus() allows changing the downstream response
// HTTP status code before sending it to the client.
//
// This function should be used in the header_filter phase,
// as sigma is preparing headers to be sent back to the client.
func (r Response) SetStatus(status int) error {
	_, err := r.Ask(`sigma.response.set_status`, status)
	return err
}

// sigma.Response.SetHeader() sets a response header with the given value.
// This function overrides any existing header with the same name.
//
// This function should be used in the header_filter phase,
// as sigma is preparing headers to be sent back to the client.
func (r Response) SetHeader(k string, v string) error {
	_, err := r.Ask(`sigma.response.set_header`, k, v)
	return err
}

// sigma.Response.AddHeader() adds a response header with the given value.
// Unlike sigma.Response.SetHeader(), this function does not remove
...
// with the given value, similarly to sigma.Response.SetHeader().
//
// This function should be used in the header_filter phase,
// as sigma is preparing headers to be sent back to the client.
func (r Response) AddHeader(k string, v string) error {
	_, err := r.Ask(`sigma.response.add_header`, k, v)
	return err
}

// sigma.Response.ClearHeader() removes all occurrences of the specified header
// in the response sent to the client.
//
// This function should be used in the header_filter phase,
// as sigma is preparing headers to be sent back to the client.
func (r Response) ClearHeader(k string) error {
	_, err := r.Ask(`sigma.response.clear_header`, k)
	return err
}

// sigma.Response.SetHeaders() sets the headers for the response.
// Unlike sigma.Response.SetHeader(), the headers argument must be a map
...
func (r Response) SetHeaders(headers map[string][]string) error {
	_, err := r.Ask(`sigma.response.set_headers`, headers)
	return err
}

// sigma.Response.Exit() interrupts the current processing and produces a response.
// It is typical to see plugins using it to produce a response before sigma
...
//
// This function closes the channel back to sigma main process, so any use of a
...
//
// Calling `sigma.Response.Exit()` will interrupt the execution flow of
// plugins in the current phase. Subsequent phases will still be invoked.
// E.g. if a plugin called `sigma.Response.Exit()` in the `access` phase, no
...
// If non nil, its behavior is similar to `sigma.response.set_headers()`.
//
...
func (r Response) Exit(status int, body string, headers map[string][]string) {
	r.AskClose(`sigma.response.exit`, status, body, headers)
}

// sigma.Response.ExitStatus() terminates current processing just like sigma.Response.Exit()
// without setting the body or headers.
func (r Response) ExitStatus(status int) {
	r.AskClose(`sigma.response.exit`, status)
}
