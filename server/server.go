/*
Package sigma/go-pdk/server implements an embedded plugin server.
...
var (
	sigmaPrefix = flag.String("sigma-prefix", "/usr/local/sigma", "sigma prefix path (specified by the -p argument commonly used in the sigma cli)")
...
func getSocketPath() (pth string, err error) {
	...
	pth = path.Join(*sigmaPrefix, name+".socket")
	return
}
