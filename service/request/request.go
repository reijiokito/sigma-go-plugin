...
// Holds this module's functions.  Accessible as `sigma.ServiceRequest`
type Request struct {
	bridge.PdkBridge
}
...
// sigma.ServiceRequest.SetScheme() sets the protocol to use
// when proxying the request to the Service.
// Supported values are "http" or "https".
func (r Request) SetScheme(scheme string) error {
	_, err := r.Ask(`sigma.service.request.set_scheme`, scheme)
	return err
}

// sigma.ServiceRequest.SetPath() sets the path component
...
func (r Request) SetPath(path string) error {
	_, err := r.Ask(`sigma.service.request.set_path`, path)
	return err
}

// sigma.ServiceRequest.SetRawQuery() sets the querystring
...
// see sigma.ServiceRequest.SetQuery().
func (r Request) SetRawQuery(query string) error {
	_, err := r.Ask(`sigma.service.request.set_raw_query`, query)
	return err
}

// sigma.ServiceRequest.SetMethod() sets the HTTP method
...
func (r Request) SetMethod(method string) error {
	_, err := r.Ask(`sigma.service.request.set_method`, method)
	return err
}

// sigma.ServiceRequest.SetQuery() sets the querystring of the request to the Service.
//
// Unlike sigma.ServiceRequest.SetRawQuery(), the query argument must be a map
...
// can be given as a string with sigma.ServiceRequest.SetRawQuery().
func (r Request) SetQuery(query map[string][]string) error {
	_, err := r.Ask(`sigma.service.request.set_query`, query)
	return err
}

// sigma.ServiceRequest.SetHeader() sets a header in the request
...
func (r Request) SetHeader(name string, value string) error {
	_, err := r.Ask(`sigma.service.request.set_header`, name, value)
	return err
}

// sigma.ServiceRequest.AddHeader() adds a request header with the given value
// to the request to the Service. Unlike sigma.ServiceRequest.SetHeader(),
...
func (r Request) AddHeader(name string, value string) error {
	_, err := r.Ask(`sigma.service.request.add_header`, name, value)
	return err
}

// sigma.ServiceRequest.ClearHeader() removes all occurrences
// of the specified header in the request to the Service.
func (r Request) ClearHeader(name string) error {
	_, err := r.Ask(`sigma.service.request.clear_header`, name)
	return err
}

// sigma.ServiceRequest.SetHeaders() sets the headers of the request
// to the Service. Unlike sigma.ServiceRequest.SetHeader(), the headers argument
...
func (r Request) SetHeaders(headers map[string][]string) error {
	_, err := r.Ask(`sigma.service.request.set_headers`, headers)
	return err
}

// sigma.ServiceRequest SetRawBody() sets the body of the request to the Service.
//
...
// see sigma.ServiceRequest.SetBody().
func (r Request) SetRawBody(body string) error {
	_, err := r.Ask(`sigma.service.request.set_raw_body`, body)
	return err
}
