...
// Holds this module's functions.  Accessible as `sigma.Service`
type Service struct {
	bridge.PdkBridge
}
...
// sigma.Service.SetUpstream() sets the desired Upstream entity to handle
...
func (s Service) SetUpstream(host string) error {
	_, err := s.Ask(`sigma.service.set_upstream`, host)
	return err
}

// sigma.Service.SetTarget() sets the host and port on which to connect to
// for proxying the request.
//
// Using this method is equivalent to ask sigma to not run the load-balancing
...
func (s Service) SetTarget(host string, port int) error {
	_, err := s.Ask(`sigma.service.set_target`, host, port)
	return err
}
